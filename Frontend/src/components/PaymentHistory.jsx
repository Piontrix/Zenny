		...
		const fetchPayments = async () => {
			try {
				...
				if (user.role === "creator") {
					endpoint = API.GET_CREATOR_PAYMENTS;
				} else if (user.role === "editor") {
					endpoint = API.GET_EDITOR_PAYMENTS;
				} else {
					setPayments([]);
					return;
				}
					...
					<table className="min-w-full bg-white border border-gray-200 rounded-lg shadow">
						<thead>
							<tr className="bg-gray-50 border-b">
								...
								<th className="px-4 py-2 text-left text-sm font-medium text-gray-600">Counterparty</th>
							</tr>
						</thead>
						<tbody>
							{payments
								...
								.map((p) => {
									let counterparty = "—";
									if (user.role === "creator") {
										counterparty = `Payment made to ${p?.editor?.username || "Unknown"}`;
									} else if (user.role === "editor") {
										counterparty = `Payment received from ${p?.creator?.username || "Unknown"}`;
									}
									...
									return (
										<tr key={p._id} className="border-b hover:bg-gray-50">
											...
											<td className="px-4 py-2 text-sm text-gray-700">{counterparty}</td>
										</tr>
									);
								})}
						</tbody>
